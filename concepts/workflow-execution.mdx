...
- **Triggers**: A trigger defines when and how workflows are activated. Triggers listen to specific events, conditions, or time intervals to initiate workflow execution. Learn more about [Kwala workflow triggers](/concepts/triggers).
...
- **Action**: Defines what happens when the trigger fires and conditions are met. Actions can include calling smart contracts on any supported chain, sending webhooks to external APIs, transferring tokens between addresses, or coordinating multiple operations across different networks. Learn more about [Kwala workflow actions](/concepts/actions).
