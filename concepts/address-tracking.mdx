...
The following example shows a complete address tracking workflow that demonstrates both static and dynamic notification patterns. This workflow monitors an address on Polygon (chain ID `137`) and fires two actions whenever activity is detected:
...
<Steps>
  <Step title="Deploy or pick a contract address">
    Use an existing contract address on chain `137` (Polygon) or deploy a new one. The example workflow uses `0x18fcd17b107348bd54c05da39cf53621b892bcac`.
  </Step>
  ...
  <Step title="Verify against a block explorer">
    Cross-reference the `txHash` and `blockNumber` values from the dynamic payload against [PolygonScan](https://www.polygonscan.com/) or your target chain's explorer to confirm accuracy.
  </Step>
</Steps>
