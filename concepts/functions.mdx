...
```yaml
Name: deploycontract-kwala-qa-02
...
The event signature, which represents the computational transformations for the example above would be:
...
This workflow deploys the Kwala function logic onto Kwala's decentralized ledger. The Go contract evaluates token transfers and emits `TokenTransferEvaluated` events for high-value transactions.
...
This workflow listens to the `TokenTransferEvaluated` event emitted by the Kwala function and executes an external action (Telegram notification) using the enriched event data.

**YAML 3: Event listener workflow**

```yaml
Name: event_listen_deploy_kwala_01
Trigger:
  ...
  RecurringEventName: TokenTransferEvaluated(from string, to string, amount number, category string)
  RepeatEvery: event
  ExecuteAfter: event
