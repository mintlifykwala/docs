...
| API_KEY      | A key supplied in a request header.                |
| JWT          | A signed JSON Web Token validated on each request. |

<Note>
Use `None` only for testing or public-safe endpoints. For any production endpoint that performs on-chain actions, always use `Basic`, `Bearer Token`, `API_KEY`, or `JWT`.
</Note>
...
**3. PUT with Bearer Token authentication** — carries a full-resource payload, requires a bearer token in the `Authorization` header, returns `200 — OK`.

```yaml
Trigger:
TriggerAPIType: PUT
TriggerAPIAuthType: Bearer Token
TriggerAPIAuthConfig:
...
**4. PATCH with API_KEY authentication** — carries a partial-update payload, requires an API key in a request header, returns `202 — Accepted`.

```yaml
Trigger:
TriggerAPIType: PATCH
TriggerAPIAuthType: API_KEY
TriggerAPIAuthConfig:
  header: "x-api-key"
  key: "your-api-key"
...
| **TriggerAPIAuthType**         | Required | One-time  | `NONE`, `Basic`, `Bearer Token`, `API_KEY`, or `JWT`    |
