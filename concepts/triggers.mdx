...
The following example demonstrates a basic event-based trigger that monitors a smart contract on Optimism mainnet for `Transfer` events. Whenever tokens are transferred from the specified contract, this trigger will activate the workflow.
...
The following example demonstrates cross-chain monitoring by tracking related events on both Optimism and Polygon. This is useful for bridge monitoring, cross-chain messaging, or coordinating actions across multiple networks.

```yaml
trigger:
  # Primary trigger on Optimism
  triggerChainID: 10
