...
- Listens to a smart contract event on-chain. Here, the Polygon chain, Mainnet, is used.
...
Deploy the contract using the Remix IDE and connect your Metamask wallet to **Polygon Mainnet**.

<Note>
You must have POL Faucet to deploy the smart contract.
</Note>

Once successfully deployed, copy your deployed smart **contract address** from the Polygon explorer.

(Refer to `https://www.polygonscan.com/` to check the deployed smart contract and related transactions.)
...
- **Trigger Chain ID**: `137` here: Polygon Mainnet.
- **Source Contract Address**: for example, `0x8ff6dEe7e8860146E51029E6116096239D8006ed` the address of the deployed smart contract, available on the Polygonscan website.
...
<Warning>
Make sure that you're in the Kwala network (not Polygon Mainnet) in your MetaMask to implement this.
</Warning>
...
<Warning>
Make sure you're in the Polygon Mainnet in your MetaMask when executing the contract via Remix IDE.
</Warning>
