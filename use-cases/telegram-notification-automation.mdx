...
- **Execute After**: Set to `event`: executes the action after an event is emitted, for example, `immediate`.
...
Now save and compile this YAML workflow code.

After successful compilation, deploy it.
...
- **Retries & Failures** – to debug if notifications didn't reach

## Conclusion

With just a few steps, we've created a real-time blockchain-to-Telegram notification system using Kwala Workflows: no servers, no SDKs, just YAML automation.

This setup shows how developers can easily integrate Web3 event triggers with Web APIs, enabling system alerts, user notifications, or smart contract monitors with minimal effort.
