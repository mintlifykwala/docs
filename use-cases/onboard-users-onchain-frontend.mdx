...
1. MetaMask (or compatible wallet) connected to the target RPC. This guide uses the Polygon mainnet. 
...
const provider = new ethers.providers.JsonRpcProvider("https://rpc.polygon.technology");
...
3. **Trigger:**
   ...
   - **Recurring Chain ID:** `137` (Polygon)
...
<CardGroup cols={2}>
  ...
  <Card title="Contract Explorer" icon="magnifying-glass">
    Verify `RecordSaved` events and transaction receipts on Polygonscan
  </Card>
</CardGroup>
