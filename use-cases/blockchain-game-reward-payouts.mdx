...
- **TRIGGER:** Detects a `SlidePerformed(address)` event emitted by the `SlideBounty` smart contract on Polygon mainnet.
...
<Warning>
After deployment, fund the contract with test POL so it can pay bounties. You can view it on Polygonscan.
</Warning>
...
5. Verify `BountyPaid` events on Polygonscan and check Telegram for the success message:
