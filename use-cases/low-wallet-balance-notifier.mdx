...
- Monitors a wallet balance on the Polygon mainnet.
...
- A MetaMask wallet connected to the Polygon mainnet and Kwala network.
...
<Note>
Deploy the contract using Remix and connect your MetaMask wallet to Polygon Mainnet. You must have POL Faucet to deploy the smart contract.
</Note>

After deployment, copy your deployed smart contract address from the Polygon explorer (`https://polygonscan.com/`) to inspect the contract and its transactions.
...
- **ParameterValue: RecurringSource Contract:** `0x1C5a23Fd1F7fEd9A5d020858BD4b93e5c93C92a8` the deployed smart contract address, available on Polygonscan.
- **Trigger Chain ID:** `137` Polygon Mainnet
...
<Warning>
Make sure you are in the Kwala network (not Polygon mainnet) in MetaMask when creating/deploying this workflow.
</Warning>
...
Provide the wallet address that has Polygon mainnet faucet tokens when calling the function.
