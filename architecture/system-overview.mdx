...
- **Execution nodes**: Monitor triggers, process workflows, and execute actions across blockchain networks. They generate cryptographic execution proofs containing all inputs, outputs, and state transitions, which are submitted to Kwala's decentralized ledger.
...
All verification activity is permanently recorded on Kwala's decentralized ledger for complete transparency and auditability. This includes intent hashes (cryptographic hashes of user-signed workflow definitions), execution logs (detailed records of every action), verifier reports (independent attestations or disputes), and Merkle proofs (cryptographic links between execution logs and on-chain records).
...
All workflow executions are traceable to a user-signed intent, and logs are publicly auditable on Kwala's decentralized ledger. Every action taken by the network can be cryptographically verified against the original workflow definition.
