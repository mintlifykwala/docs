...
<Warning>
  **Deprecated networks.** Kwala no longer supports the following chains: Avalanche Fuji Testnet, Polygon Testnet, Ethereum Testnet and Mainnet, OP Sepolia Testnet, Base Sepolia Testnet, BNB Testnet, and Celo Sepolia Testnet.
</Warning>
...
<Frame>
  <img src="/images/workflow-builder/configure-workflow/configure-triggers-new.png" alt="Trigger configuration settings" />
</Frame>

### Required trigger fields

The required fields for trigger configuration are **Execute after**, **Repeat every**, and **Expires in**.
...
<Frame>
  <img src="/images/workflow-builder/configure-workflow/oracle-price-trigger-new.png" alt="Oracle price trigger source configuration" />
</Frame>
...
Similar to the **Execute after** trigger, selecting a **Repeat every** trigger displays additional configuration fields. 

For example, if you select **Event** as the **Repeat every** trigger, the builder displays a Recurring Source Configuration as shown:

<Frame>
  <img src="/images/workflow-builder/configure-workflow/trigger-config-repeat-after-event-new.png" alt="Recurring source configuration for event-based repeat" />
</Frame>

- **Recurring Contract**: The smart contract address to monitor for recurring events. For example, `0x49e833337ece7afe375e44f4e3e8481029218e5c`.
...
<Frame>
  <img src="/images/workflow-builder/configure-workflow/calendar-date-picker-new.png" alt="Expires in calendar picker" />
</Frame>
...
<Frame>
  <img src="/images/workflow-builder/configure-workflow/configure-actions-new.png" alt="Action configuration settings" />
</Frame>

Required fields for POST actions:

- **Webhook URL**: The URL to send the request to. For example, `https://api.telegram.org/bot7754368882:AAHS4KbbOkl5rEHoBBIR8eljgwq2PARYLyY/sendMessage`
- **API Payload**: The JSON data to include in the request body. You can pass both fixed values and event parameters in a POST action. 
...
```yaml
    ...
    Type: post
    APIEndpoint: https://your-endpoint.example.com/webhook
    APIPayload:
...
**Web3 smart contract function**

Use the **Web3 Call** action type to interact with an existing smart contract by calling one of its functions.

<Frame>
  <img src="/images/workflow-builder/configure-workflow/configure-call-actions-new.png" alt="CALL action configuration" />
</Frame>

Required fields for Web3 Call actions:

- **Target Contract**: The address of the smart contract to interact with. For example, `0x65b2b850fee0e0c13d956ab5...`
- **Function**: The function signature to call. For example, `function updateNFT(string memory userId, uint256 points)`
- **Chain ID**: The blockchain network where the contract is deployed. Select from the dropdown
- **Params**: The parameters to pass to the function. You can use fixed values or reference event data dynamically with re.event(0), re.event(1), etc. Enter each parameter on a separate line. 
- **Retries Until Success**: Number of retry attempts if the call fails

**Example Web3 Call configuration**
...
<Frame>
  <img src="/images/workflow-builder/configure-workflow/configure-deploy-action-new.png" alt="Deploy action configuration" />
</Frame>
...
<Frame>
  <img src="/images/workflow-builder/configure-workflow/configure-execution-mode-new.png" alt="Execution mode selection" />
</Frame>
